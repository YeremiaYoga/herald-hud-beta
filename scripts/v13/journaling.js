...
async function heraldHud_renderListPersonalNotesMiddleContainer() {
  ...
  for (let journal of filteredPersonalNotes) {
    ...
    console.log(journal.flags["herald-hud"]);
    let type = journal.flags["herald-hud"]?.type || "";
          ...
          buttons: {
            save: {
              ...
              callback: async (html) => {
                ...
                journal.flags["herald-hud"] = journal.flags["herald-hud"] || {};
                journal.flags["herald-hud"].type = finalType;
...
async function heraldHud_renderListQuest() {
  ...
  for (let quest of allQuestJournals) {
    ...
    if (
      activeTypeEl &&
      quest.flags["herald-hud"].type !== activeTypeEl.dataset.name
    )
      continue;
    let visibleButton = ``;
    let deleteButton = ``;
    if (user.isGM) {
      const isVisible = quest.flags["herald-hud"].visible;
      const iconColor = isVisible == "on" ? "white" : "gray";
    ...
    `;
      ...
      if (quest.flags["herald-hud"].visible == "off") {
        continue;
      }
    }
    let src = "";
    for (let t of typeQuest) {
      if (quest.flags["herald-hud"].type == t.name) {
        src = t.src;
      }
    }
    ...
    // const src = typeQuest.find(t => t.name === quest.flags["herald-hud"].type)?.src || "";
  ...
  if (listQuestView) {
    ...
    visible.forEach((vsb) => {
      vsb.addEventListener("click", async () => {
        ...
        const currentVisible = vsb.getAttribute("data-visible");

        const journal = game.journal.get(questId);
        console.log(journal);
        if (!journal) return;
        let newVisible = "on";
        if (currentVisible == "on") {
          newVisible = "off";
        }
        ...
        // let allFlags = duplicate(journal.flags);
        // allFlags["herald-hud"] = allFlags["herald-hud"] || {};
        // allFlags["herald-hud"].visible = newVisible;

        // await journal.update({ flags: allFlags });

        console.log(journal);
        vsb.setAttribute("data-visible", newVisible);
        const icon = vsb.querySelector("i");
        icon.style.color = newVisible == "on" ? "white" : "gray";
...
async function heraldHud_renderQuestPage() {
  ...
  const matchType = typeQuest.find(
    (t) => t.name === journal.flags["herald-hud"].type
  );
  let src = matchType ? matchType.src : "";
